export * from './icons';
export * from './sliders';
export * from './shales';

export { default as logoImage } from './logo.svg';
export { default as logoTextImage } from './logo_text.svg';
export { default as mainImage } from './main_img.png';
export { default as sliderBlockBG } from './sliderBlockBG.svg';
export { default as sliderBlockLineBG } from './sliderBlockLineBG.svg';
export { default as placePowerImage } from './placePower.png';
export { default as placePowerManImage } from './placePowerMan.png';
export { default as placePowerManBigImage } from './placePowerManBigImage.png';
export { default as timeBg } from './timeBg.svg';
export { default as timeOpenImg } from './timeOpen.png';
export { default as timeBeBe } from './timeBeBe.png';
export { default as transferCarImg } from './transferCar.png';
export { default as transferCar2Img } from './transferCar2.png';
